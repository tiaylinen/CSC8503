...
bool CollisionDetection::SpherePlaneIntersection(const SphereVolume& volume, const Transform& worldTransform, const Plane& p, CollisionInfo& collisionInfo) {
	...
	float distance = (planePoint - spherePos).Length();

	if (distance < sphereRadius) {
		float penetration = (sphereRadius - distance);
		Vector3 normal = spherePos.Normalised();
		Vector3 localA = normal * volume.GetRadius();
		Vector3 localB = p.GetPointOnPlane();
